...
type Trie interface {
	...
	GetKey([]byte) []byte // TODO(fjl): remove this when TradingTrie is removed
...
type cachingDB struct {
	db            *trie.Database
	mu            sync.Mutex
	pastTries     []*TradingTrie
	codeSizeCache *lru.Cache
}

// OpenTrie opens the main account trie.
func (db *cachingDB) OpenTrie(root common.Hash) (Trie, error) {
	return NewTradingTrie(root, db.db)
}

func (db *cachingDB) pushTrie(t *TradingTrie) {
	db.mu.Lock()
	defer db.mu.Unlock()
