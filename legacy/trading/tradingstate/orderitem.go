...
func VerifyBalance(relayerSMC common.Address, statedb *state.StateDB, tradingStatedb *TradingStateDB, order *types.OrderTransaction, baseDecimal, quoteDecimal *big.Int) error {
	var quotePrice *big.Int
	if order.QuoteToken().String() != NativeTokenAddress {
		quotePrice = tradingStatedb.GetLastPrice(GetTradingOrderBookHash(order.QuoteToken(), common.HexToAddress(NativeTokenAddress)))
		log.Debug("TryGet quotePrice QuoteToken/ETH", "quotePrice", quotePrice)
		if quotePrice == nil || quotePrice.Sign() == 0 {
			inversePrice := tradingStatedb.GetLastPrice(GetTradingOrderBookHash(common.HexToAddress(NativeTokenAddress), order.QuoteToken()))
