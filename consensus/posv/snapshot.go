...
func (s *Snapshot) apply(headers []*types.Header) (*Snapshot, error) {
	...
	for i, header := range headers {
		...
		if time.Since(logged) > 8*time.Second {
			log.Info("[POSV] Reconstructing voting history", "processed", i, "total", len(headers), "elapsed", common.PrettyDuration(time.Since(start)))
			logged = time.Now()
		}
	}
	if time.Since(start) > 8*time.Second {
		log.Info("[POSV] Reconstructed voting history", "processed", len(headers), "elapsed", common.PrettyDuration(time.Since(start)))
	}
