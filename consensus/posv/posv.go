...
func (c *Posv) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header) error {
...
func (c *Posv) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header) (chan<- struct{}, <-chan error) {
...
func (c *Posv) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
	...
	if chain.Config().IsShanghai(header.Number, header.Time) {
		return errors.New("posv does not support shanghai fork")
	}
	...
	if chain.Config().IsCancun(header.Number, header.Time) {
		return errors.New("posv does not support cancun fork")
	}
...
func (c *Posv) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
	...
	if number%c.config.Epoch == 0 {
		...
		err := c.verifyValidators(chain, header, parents, log.Root())
		if err != nil {
			return err
		}
	}
	// All basic checks passed, verify the seal and return
	return c.verifySeal(chain, snap, header, parents, log.Root())
}
...
// consensus rules in posv, do nothing here.
func (c *Posv) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state vm.StateDB, body *types.Body) {
	// No block rewards in PoA, so the state remains as is
}

// FinalizeAndAssemble implements consensus.Engine, ensuring no uncles are set,
// nor block rewards given, and returns the final block.
func (c *Posv) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, body *types.Body, receipts []*types.Receipt) (*types.Block, error) {
	if len(body.Withdrawals) > 0 {
		return nil, errors.New("posv does not support withdrawals")
	}
	// Finalize block
...
// with.
func (c *Posv) Authorize(signer common.Address) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *Posv) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	panic("posv (poa) sealing not supported any more")
}
...
func encodeSigHeader(w io.Writer, header *types.Header) {
	...
	if header.WithdrawalsHash != nil {
		panic("unexpected withdrawal hash value in posv")
	}
	if header.ExcessBlobGas != nil {
		panic("unexpected excess blob gas value in posv")
	}
	if header.BlobGasUsed != nil {
		panic("unexpected blob gas used value in posv")
	}
	if header.ParentBeaconRoot != nil {
		panic("unexpected parent beacon root value in posv")
	}
