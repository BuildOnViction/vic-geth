...
func (p *StateProcessor) CommitLendingState(block *types.Block) error {
	...
	log.Info("[NativeLending] Flushed Trie to disk", "block", block.NumberU64(), "root", lendingRoot.Hex())
	return nil
}
...
func (p *StateProcessor) CommitLendingStateDeferred(block *types.Block) error {
	...
	log.Info("[NativeLending] Flushed Trie to disk", "block", current, "root", lendingRoot.Hex())
...
func (p *StateProcessor) CommitTradingState(block *types.Block) error {
	...
	log.Info("[NativeTrading] Flushed Trie to disk", "block", block.NumberU64(), "root", tradingRoot.Hex())
	return nil
}
...
func (p *StateProcessor) CommitTradingStateDeferred(block *types.Block) error {
	...
	log.Info("[NativeTrading] Flushed Trie to disk", "block", current, "root", tradingRoot.Hex())
