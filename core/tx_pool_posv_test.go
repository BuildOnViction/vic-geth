...
func TestApplySignTransaction_NonceMismatch(t *testing.T) {
	...
	_, receipt, _, err, _ := ApplySignTransaction(testPosvConfig, statedb, tx, header, &usedGas)
	if err == nil {
	...
	_, receipt2, _, err2, _ := ApplySignTransaction(testPosvConfig, statedb, tx2, header, &usedGas)
	if err2 == nil {
	...
	_, receipt3, _, err3, _ := ApplySignTransaction(testPosvConfig, statedb, tx3, header, &usedGas)
	if err3 != nil {
